# pan115-scraper Windows构建脚本 (PowerShell)
# 用于GitHub Actions和本地Windows构建

Write-Host "🚀 开始构建 pan115-scraper (Windows)..."
...
# 检查必要文件
Write-Host "📋 检查必要文件..."
$requiredFiles = @("app.py", "requirements.txt", "pan115-scraper.spec")
...
if (-not (Test-Path "templates/index.html")) {
    ...
    "<html><body><h1>pan115-scraper</h1><p>请确保正确的模板文件存在</p></body></html>" | Out-File -FilePath "templates/index.html" -Encoding utf8
}

# 检查静态文件
if (-not (Test-Path "static/style.css")) {
    Write-Host "⚠️  警告: static/style.css 不存在，创建占位符文件"
    "/* pan115-scraper styles - 请确保正确的CSS文件存在 */" | Out-File -FilePath "static/style.css" -Encoding utf8
}

if (-not (Test-Path "static/script.js")) {
    Write-Host "⚠️  警告: static/script.js 不存在，创建占位符文件"
    "// pan115-scraper scripts - 请确保正确的JS文件存在" | Out-File -FilePath "static/script.js" -Encoding utf8
}
...
# 验证spec文件
if (-not (Test-Path "pan115-scraper.spec")) {
    Write-Host "❌ 错误: pan115-scraper.spec 不存在"
    exit 1
}
...
if (Test-Path "dist") {
    ...
    if (Test-Path "dist/pan115-scraper-win.exe") {
        Write-Host "✅ 可执行文件构建成功: dist/pan115-scraper-win.exe"
        $fileSize = (Get-Item "dist/pan115-scraper-win.exe").Length
        $fileSizeMB = [math]::Round($fileSize / 1MB, 2)
        Write-Host "📊 文件大小: $fileSizeMB MB"
    } else {
        Write-Host "❌ 错误: 预期的可执行文件不存在: dist/pan115-scraper-win.exe"
        Write-Host "📋 dist目录内容:"
...
if (Test-Path "dist/pan115-scraper-win.exe") {
    ...
    try {
        $process = Start-Process -FilePath "dist/pan115-scraper-win.exe" -ArgumentList "--help" -PassThru -WindowStyle Hidden
...
Write-Host ""
Write-Host "🎉 构建完成！"
Write-Host "📁 可执行文件位置: dist/pan115-scraper-win.exe"
Write-Host "📊 构建统计:"
