...
# 检查必要文件
echo "📋 检查必要文件..."
required_files=("app.py" "requirements.txt" "pan115-scraper.spec")
...
if [ -d "dist" ]; then
    ...
    if [[ "$OSTYPE" == "msys" || "$OSTYPE" == "win32" ]]; then
        expected_exe="dist/pan115-scraper-win.exe"
    elif [[ "$OSTYPE" == "darwin"* ]]; then
        expected_exe="dist/pan115-scraper-mac"
    else
        expected_exe="dist/pan115-scraper-linux"
    fi
