...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('scrapePreviewBtn disabled:', scrapePreviewBtn ? scrapePreviewBtn.disabled : 'element not found');
    ...
    const showProcessedCheckbox = document.getElementById('showProcessedCheckbox');
    ...
    function showOperationResultModal(content) {
        ...
        if (!content || content.trim() === '') {
            console.warn('showOperationResultModal: 内容为空，跳过显示弹窗');
            return;
        }
        ...
        if (textContent.trim() === '') {
            console.warn('showOperationResultModal: HTML内容为空，跳过显示弹窗');
            return;
        }
    ...
    // 页面顶部消息显示函数
    function showPageMessage(content, type = 'success', duration = 5000) {
        console.log('showPageMessage: 显示页面消息', {
        ...
        // 检查内容是否有效
        if (!content || content.trim() === '') {
            console.warn('showPageMessage: 内容为空，跳过显示');
            return false;
        }
        ...
        if (!messageContainer || !messageContent) {
            console.error('showPageMessage: 找不到消息容器元素');
            return false;
        }
        ...
        console.log('showPageMessage: 消息已显示');
        return true;
    }
    ...
    function hidePageMessage() {
        ...
        if (messageContainer) {
            ...
            console.log('hidePageMessage: 消息已隐藏');
        }
    }

    // 确保弹窗正确显示的辅助函数
    function ensureModalDisplayed(contentElement, modalElement, content) {
        console.log('ensureModalDisplayed: 开始处理弹窗显示', {
        ...
        // 检查内容是否有效
        if (!content || content.trim() === '') {
            console.warn('ensureModalDisplayed: 内容为空，跳过显示');
            return false;
        }
        ...
        const textContent = tempDiv.textContent || tempDiv.innerText || '';
        if (textContent.trim() === '') {
            console.warn('ensureModalDisplayed: HTML内容为空，跳过显示');
            return false;
        }

        // 优先使用传入的元素，如果没有则重新获取
        const operationResultContent = contentElement || operationResultsDiv || document.getElementById('operationResults');
        const operationResultModal = modalElement || document.getElementById('operationResultModal');

        console.log('ensureModalDisplayed: DOM元素状态', {
            hasOperationResultContent: !!operationResultContent,
            hasOperationResultModal: !!operationResultModal,
            modalCurrentDisplay: operationResultModal ? operationResultModal.style.display : 'null'
        });

        if (operationResultContent && operationResultModal) {
            // 更新内容
            operationResultContent.innerHTML = content;

            // 确保弹窗显示
            if (operationResultModal.style.display !== 'block') {
                operationResultModal.style.display = 'block';
                document.body.style.overflow = 'hidden';
                console.log('ensureModalDisplayed: 弹窗已显示');
            } else {
                console.log('ensureModalDisplayed: 弹窗已经是显示状态，仅更新内容');
            }

            console.log('ensureModalDisplayed: 弹窗内容已更新并确保显示');
            return true;
        } else {
            // 最后的备用方案：直接使用alert
            console.error('ensureModalDisplayed: 无法找到弹窗元素，使用alert显示');
            alert('操作完成，但无法显示详细结果');
            return false;
        }
    }
    ...
    const aiApiKeyInput = document.getElementById('geminiApiKeyInput');
    const aiApiUrlInput = document.getElementById('geminiApiUrlInput');
    ...
    const ScrapeResultFilter = {
        // 过滤器配置
        filters: {
            showProcessed: {
                enabled: false,
                condition: (result) => result.status !== SCRAPE_STATUS.ALREADY_PROCESSED
            }
        },
        ...
        applyFilters(results) {
            ...
            Object.entries(this.filters).forEach(([key, filter]) => {
                if (!filter.enabled && filter.condition) {
                    filteredResults = filteredResults.filter(filter.condition);
                }
            });
        ...
        // 获取过滤器状态
        getFilterStatus(filterName) {
            return this.filters[filterName]?.enabled || false;
        }
    };
    ...
    async function fetchFolderContent(folderId) {
        ...
        try {
            const response = await fetch('/get_folder_content', {
                ...
                body: `cid=${encodeURIComponent(folderId)}&limit=${limitInput ? limitInput.value : 100}`
            });
            const data = await response.json();
    ...
    function renderFolderContent(items) {
        ...
        sortedItems.forEach(item => {
            ...
            if (item.is_dir) {
                ...
                row.dataset.fileId = item.fileId; // 添加data-file-id属性供右键菜单使用
                nameHtml = `
                ...
                row.dataset.cid = item.cid || item.fileId;
            } else {
    ...
    function updatePathLinks(pathParts) {
        ...
        pathParts.forEach((part) => {
            ...
            pathHistory.push({ name: displayName, cid: part.cid });
        });
        ...
        pathHistory.forEach((pathItem, index) => {
            ...
            link.addEventListener('click', (e) => {
                e.preventDefault();
                fetchFolderContent(pathItem.cid);
            });
    ...
    async function fetchConfig() {
        try {
            ...
            if (response.ok) {
                ...
                if (cookiesInput) cookiesInput.value = config.COOKIES || '';

                if (tmdbApiKeyInput) tmdbApiKeyInput.value = config.TMDB_API_KEY || '';
                if (aiApiKeyInput) aiApiKeyInput.value = config.GEMINI_API_KEY === '********' ? '' : config.GEMINI_API_KEY || '';
                if (aiApiUrlInput) aiApiUrlInput.value = config.GEMINI_API_URL || '';
    ...
    configForm.addEventListener('submit', async (e) => {
        ...
        const configData = {
            ...
            COOKIES: cookiesInput ? cookiesInput.value : '',

        };

        // 添加可选的配置项
        if (tmdbApiKeyInput && tmdbApiKeyInput.value) configData.TMDB_API_KEY = tmdbApiKeyInput.value;
        if (aiApiKeyInput && aiApiKeyInput.value) configData.GEMINI_API_KEY = aiApiKeyInput.value;
        if (aiApiUrlInput && aiApiUrlInput.value) configData.GEMINI_API_URL = aiApiUrlInput.value;
    ...
    scrapePreviewBtn.addEventListener('click', async () => {
        ...
        console.log('Button disabled state:', scrapePreviewBtn.disabled); // 调试按钮状态
        ...
        // 收集所有选中的项目，包括文件和文件夹（参考movie115的实现）
        const itemsToScrape = selectedItemElements.map(row => {
            const id = row.dataset.id;
            const type = row.dataset.type;

            console.log('Processing row:', { id, type }); // 调试信息

            // 从currentFiles中查找完整的项目信息
            return currentFiles.find(item =>
                (type === 'file' && item.fileId === id) ||
                (type === 'folder' && item.fileId === id)
            );
        }).filter(Boolean); // 过滤掉未找到的项目

        console.log('itemsToScrape.length:', itemsToScrape.length); // 调试信息
        console.log('itemsToScrape:', itemsToScrape); // 调试信息

        if (itemsToScrape.length === 0) {
            showStatus(scrapePreviewStatus, '没有有效文件或文件夹可供刮削。', 'error');
            return;
        }
        ...
        try {
            const formData = new FormData();
            formData.append('items', JSON.stringify(itemsToScrape));
            ...
            if (data.success) {
                ...
                // 构建详细的状态消息
                const totalFiles = data.total_files || currentScrapedResults.length;
                const successCount = data.successful_matches || 0;
                const failedCount = data.failed_matches || 0;

                let statusMessage = `刮削完成: 共 ${totalFiles} 个文件`;
                if (successCount > 0) {
                    statusMessage += `, 成功 ${successCount} 个`;
                }
                if (failedCount > 0) {
                    statusMessage += `, 失败 ${failedCount} 个`;
                }
                ...
                if (data.status_stats) {
                    ...
                    const details = [];
                    if (stats.no_tmdb_result) details.push(`无匹配 ${stats.no_tmdb_result}`);
                    if (stats.no_suggestion) details.push(`无建议 ${stats.no_suggestion}`);
                    if (stats.extraction_failed) details.push(`提取失败 ${stats.extraction_failed}`);
                    if (stats.error) details.push(`错误 ${stats.error}`);
                ...
                showStatus(scrapePreviewStatus, statusMessage, 'success');
    ...
    function renderScrapePreview(results) {
        try {
            ...
            // 更新过滤器状态
            const showProcessed = showProcessedCheckbox ? showProcessedCheckbox.checked : false;
            ScrapeResultFilter.updateFilter('showProcessed', showProcessed);

            // 应用过滤器
            const filteredResults = ScrapeResultFilter.applyFilters(sortedResults);

            // 更新预览计数
            updatePreviewCount(filteredResults.length);

            // 渲染表格
            renderPreviewTable(filteredResults, showProcessed);

        } catch (error) {
            console.error('渲染刮削预览时发生错误:', error);
            showPreviewError('渲染预览时发生错误，请重试。');
        }
    }

    // 更新预览计数
    function updatePreviewCount(count) {
        if (previewCountSpan) {
            previewCountSpan.textContent = count;
        }
    }

    // 渲染预览表格
    function renderPreviewTable(results, showProcessed) {
        previewTableBody.innerHTML = '';

        if (results.length === 0) {
            const message = showProcessed ? '没有刮削结果。' : '没有需要处理的文件。';
            previewTableBody.innerHTML = `<tr><td colspan="6">${message}</td></tr>`;
            return;
        }

        results.forEach(result => renderPreviewRow(result));
        finalizePreviewTable();
    }

    // 显示预览错误
    function showPreviewError(message) {
        if (previewTableBody) {
            previewTableBody.innerHTML = `<tr><td colspan="6" style="color: red; text-align: center;">${message}</td></tr>`;
        }
    }

    // 渲染单个预览行
    function renderPreviewRow(result) {
        const row = previewTableBody.insertRow();
        row.dataset.fileId = result.fid;  // 使用fid字段
        row.dataset.originalName = result.original_name;
        row.dataset.suggestedName = result.suggested_name;

        // 格式化TMDB信息
        const tmdbInfoHtml = formatTmdbInfo(result.tmdb_info, result.file_info);

        // 格式化状态
        const { statusHtml, statusTitle } = formatResultStatus(result);
        ...
        row.innerHTML = `
            ...
            <td class="filename-cell">
                <div class="filename-suggested" title="${result.suggested_name || statusTitle}">${result.suggested_name || `<span style="color: #ff4d4f;">${statusTitle}</span>`}</div>
            </td>
            <td title="${result.size || '未知'}">${formattedSize}</td>
            <td>${tmdbInfoHtml}</td>
            <td title="${statusTitle}">${statusHtml}</td>
        `;
    }

    // 格式化TMDB信息
    function formatTmdbInfo(tmdbInfo, fileInfo) {
        if (!tmdbInfo) {
            return '<span style="color: #999; font-size: 9px;">无 TMDB 信息</span>';
        }

        const title = tmdbInfo.title || tmdbInfo.name || '未知标题';
        const year = tmdbInfo.release_date ? tmdbInfo.release_date.substring(0, 4) :
                   (tmdbInfo.first_air_date ? tmdbInfo.first_air_date.substring(0, 4) : 'N/A');
        const mediaType = fileInfo && fileInfo.media_type === 'movie' ? '电影' : '剧集';
    ...
    function formatResultStatus(result) {
        const statusMap = {
            [SCRAPE_STATUS.SUCCESS]: {
                html: '<span class="status-badge status-success">成功</span>',
                title: '刮削成功'
            },
            [SCRAPE_STATUS.NO_TMDB_RESULT]: {
                html: '<span class="status-badge status-warning">无匹配</span>',
                title: result.error || '未找到TMDB匹配结果'
            },
            [SCRAPE_STATUS.NO_SUGGESTION]: {
                html: '<span class="status-badge status-warning">无建议</span>',
                title: result.error || '无法生成建议名称'
            },
            [SCRAPE_STATUS.EXTRACTION_FAILED]: {
                html: '<span class="status-badge status-error">提取失败</span>',
                title: result.error || '无法从文件名提取电影信息'
            },
            [SCRAPE_STATUS.ALREADY_PROCESSED]: {
                html: '<span class="status-badge status-already-processed">已处理</span>',
                title: result.message || '文件已包含TMDB信息'
            },
            [SCRAPE_STATUS.ERROR]: {
                html: '<span class="status-badge status-error">错误</span>',
                title: result.error || '处理时发生错误'
            },
            'no_match': { // 保持向后兼容
                html: '<span class="status-badge status-warning">无匹配</span>',
                title: result.error || '无匹配结果'
            }
        };

        const status = statusMap[result.status] || {
            html: '<span class="status-badge status-error">未知</span>',
            title: result.error || '未知状态'
        };
    ...
    function formatFileSize(size) {
        ...
        return sizeText.length > 10 ? sizeText.substring(0, 10) + '...' : sizeText;
    }
        ...
        try {
            ...
            if (data.success) {
                ...
                if (data.results && Array.isArray(data.results)) {
                    ...
                    showOperationResultModal(`<div class="alert alert-danger"><i class="fas fa-exclamation-triangle"></i> 批量重命名失败: ${data.error || '未知错误'}</div>`);
                }
            }
        } catch (error) {
    ...
    // 页面加载时获取根目录内容
    console.log('🚀 页面初始化：开始获取根目录内容，currentFolderId:', currentFolderId);
    fetchFolderContent(currentFolderId);
            ...
            if (data.success) {
                ...
                const infoHtml = `
                    ...
                    <div class="folder-info-item">
                        ...
                        <span class="folder-info-value">${data.size_str}</span>
                    </div>
                `;
        ...
        try {
            ...
            if (data.success) {
                ...
                setTimeout(() => {
                    ...
                    // 如果后端返回刷新指令或有成功的重命名，自动刷新当前文件夹
                    if (data.refresh_folder || data.success_count > 0) {
                        console.log('🔄 重命名成功，自动刷新当前文件夹');
                        fetchFolderContent(currentFolderId);
                    }
                }, 1500);
            } else {
    ...
    deleteSelectedBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (data.success) {
                showOperationResultModal(`<div class="alert alert-success"><i class="fas fa-check-circle"></i> ${data.message || '删除操作完成'}</div>`);
                ...
                showOperationResultModal(`<div class="alert alert-danger"><i class="fas fa-exclamation-triangle"></i> 删除失败: ${data.error || '未知错误'}</div>`);
            }
        } catch (error) {
            showOperationResultModal(`<div class="alert alert-danger"><i class="fas fa-exclamation-triangle"></i> 请求失败: ${error.message}</div>`);
        } finally {
    ...
    contextMenuDeleteEmpty.addEventListener('click', async () => {
        ...
        const folderName = document.querySelector(`[data-file-id="${folderId}"]`)?.querySelector('.file-name')?.textContent || `文件夹_${folderId}`;
        hideContextMenu();

        // 确认对话框
        if (!confirm(`确定要检查并删除文件夹 "${folderName}" 吗？\n\n如果该文件夹及其所有子文件夹都不包含视频文件，将被删除。\n此操作不可撤销。`)) {
            return;
        }

        try {
            // 显示加载状态 - 使用JavaScript原生弹窗
            alert('正在检查文件夹中的视频文件，请稍候...');
            ...
            if (data.success) {
                // 构建简洁的文本消息
                let message = `删除空文件夹操作完成！\n\n`;
                message += `操作摘要：\n`;
                message += `删除: ${data.deleted_count || 0} 个\n`;
                message += `保留: ${data.preserved_count || 0} 个\n`;
                message += `失败: ${data.failed_count || 0} 个\n`;

                if (data.deleted_count > 0) {
                    message += `\n已删除的空文件夹：\n`;
                    data.deleted_folders.forEach(folder => {
                ...
                if (data.preserved_count > 0) {
                    message += `\n保留的文件夹（包含视频）：\n`;
                    data.folders_with_videos.forEach(folder => {
                        message += `📹 ${folder.folder_name} (${folder.video_count} 个视频)\n`;
                    });
                }

                if (data.failed_count > 0) {
                    message += `\n处理失败的文件夹：\n`;
                    [...(data.failed_deletions || []), ...(data.failed_analysis || [])].forEach(item => {
                        message += `✗ ${item.folder_name || item.folder_id}: ${item.error}\n`;
                    });
                }

                // 使用JavaScript原生弹窗显示成功结果
                alert(message);
                ...
                // 使用JavaScript原生弹窗显示错误内容
                alert(`操作失败: ${data.error || '未知错误'}`);
            }
        } catch (error) {
            console.error('删除空文件夹请求失败:', error);
            // 使用JavaScript原生弹窗显示错误
        ...
        if (result.success) {
            resultDiv.innerHTML = `
                <div class="alert alert-success">
                    ...
                    <p><strong>消息:</strong> ${result.message || 'AI API连接测试成功'}</p>
                    <p><strong>模型:</strong> ${result.model || '未知'}</p>
                    <p><strong>响应:</strong> ${result.response || '无响应内容'}</p>
                    <p><strong>状态码:</strong> ${result.status_code || '未知'}</p>
                </div>
            `;
        } else {
            resultDiv.innerHTML = `
                <div class="alert alert-danger">
                    ...
                    ${result.status_code ? `<p><strong>状态码:</strong> ${result.status_code}</p>` : ''}
                    ${result.response_text ? `<p><strong>响应内容:</strong> ${result.response_text}</p>` : ''}
                    <div class="mt-2">
                        <strong>可能的解决方案:</strong>
                        <ul>
                            <li>检查GEMINI_API_KEY是否正确配置</li>
                            <li>检查GEMINI_API_URL是否可访问</li>
                            <li>检查MODEL模型名称是否正确</li>
                            <li>检查网络连接是否正常</li>
                        </ul>
                    </div>
                </div>
...
function displayGroupingResults(result) {
    ...
    result.movie_info.forEach((group, index) => {
        ...
        if (result.video_files && fileIds.length > 0) {
            // 通过文件ID匹配来计算实际的文件数量，兼容不同的字段名
            const videoFileIds = result.video_files.map(f => f.fid || f.fileId || f.fileID);
            actualFileCount = fileIds.filter(id => videoFileIds.includes(id)).length;
        }
    ...
    checkboxes.forEach(checkbox => {
        if (checkbox.checked) {
            ...
            if (row) {
                ...
                const fileName = row.querySelector('.file-name')?.textContent || `文件夹_${fileId}`;
                const isDir = row.querySelector('.file-type')?.textContent?.includes('文件夹') || false;
...
function updateButtonStates() {
    // 按钮状态更新逻辑（如果需要的话可以在这里添加其他按钮的状态更新）
}
